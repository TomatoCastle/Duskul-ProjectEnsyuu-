...
#define INSTR(x)   (((x) & 0x80) || (chTable[x] & ca_instr))
...
TIN *getTextBuffer(FILE *fp) {
    ...
    tip->lines = 0;
    tip->index = 0;
    tip->capacity = LINESAVE;
    tip->buf = malloc(LINESAVE + 4);
    tip->buf[0] = 0;
    tip->ungetch = 0;
    return tip;
}
...
void printTextBuffer(FILE *fp, TIN *tip) {
    if (!isEOF(tip))
        fprintf(fp, "Line#%d: %s", tip->lines, (char *)tip->buf);
...
int nextch(TIN *tip)
{
    int u = tip->ungetch;
    if (u) {
        if (u != EOF) tip->ungetch = 0;
        return u;
    }

    int ch = tip->buf[tip->index];
    if (ch) {
        tip->index++;
        return ch;
    }
    ...
    while ((b = fgetc(tip->fp)) != EOF) {
        ...
        if (x >= tip->capacity) {
            ...
            tip->capacity *= 2;
            newbuf = realloc(tip->buf, tip->capacity + 4);
            p = tip->buf = newbuf;
        }
    }
    p[x] = 0;
